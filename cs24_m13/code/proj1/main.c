...
int main(int argc, char *argv[]) {
  ...
  if (argc == 1) {
    /* If there are no additional keyword arguments read from stdin */
    input = stdin;
  }
  else if (argc == 2) {
    /* If there is a single keyword argument, attempt to read from that file */
  ...
  // Remove some items
  if (list_get_size(list) < 1) return 0;  // No point in continuing
  char *tmp;
